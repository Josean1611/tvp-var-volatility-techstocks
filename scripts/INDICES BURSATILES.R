...
rend.JAPON<-100*dailyReturn(JAPON,type="log")

NOMBRES<-c("China","EEUU","Europa","Japon","Taiwan")

rend<-cbind.xts(rend.CHINA,rend.EEUU,rend.EUROPA,rend.JAPON,rend.TAIWAN)
any(is.na(rend))
rend<-na.omit(rend)
...
PlotNPDC(dca_filtered, ca=DCA, ylim=c(-30,40))
...
colnames(DCT)[4] <-"JAPON"
colnames(DCT)[5] <-"TAIWAN"
DCT

rownames(DCT)[1] <-"CHINA"
rownames(DCT)[2] <-"EEUU"
rownames(DCT)[3] <-"EUROPA"
rownames(DCT)[4] <-"JAPON"
rownames(DCT)[5] <-"TAIWAN"
DCT
